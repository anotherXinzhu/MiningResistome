...
load("_data/nonRegArgCarryingContigs.proportions.RData")
...
dat_pubAMD_Europe <- numContig_df_pubAMD %>% 
  group_by(ecosystem,location_Eng) %>% 
  summarise(latitude=mean(latitude), n=n(),
            avg_ACCpercent_byNum=mean(ACCpercent_byNum), sd_ACCpercent_byNum=sd(ACCpercent_byNum),
            avg_ACCpercent_byDepth=mean(ACCpercent_byDepth),sd_ACCpercent_byDepth=sd(ACCpercent_byDepth),
...
dat_pubAMD_combined <-
  dat_pubAMD_combined %>% filter( !is.na(latitude)) %>% arrange(site_ecosystem) %>% arrange(latitude)
dat_pubAMD_combined$site_ecosystem <- 
  factor(dat_pubAMD_combined$site_ecosystem,levels = dat_pubAMD_combined$site_ecosystem)
...
y_vec <- c("ACC_MetalRG_Percent","ACC_MGEnzyme_Percent")
for(y in y_vec){
  ...
  p  <- ggplot(dat)+
    ...
    ylim(c(0,0.75)) +  # for ARG-MGE enzyme proportion
    scale_fill_manual(values = Colors)  +
    scale_x_discrete(labels=dat$location_Eng) + 
    xlab("") + ylab(y)+
    theme_bw() + 
...
# AMD sediment #####
dat_AMDSedi <- numContig_df_AMDSedi %>% 
  filter(!is.na(totalContigDepth)) %>%
...
# plot by variable
y_vec <- c("ACC_MetalRG_Percent_byDepth","ACC_MGEnzyme_Percent_byDepth")
for(y in y_vec){
...
# Tailing #####
dat_Tailing <- numContig_df_Tailing %>%
  filter(!is.na(totalContigDepth)) %>%
  group_by(location_Eng) %>% 
  summarise(latitude=mean(latitude), n=n(),
            avg_ACCpercent_byNum=mean(ACCpercent_byNum), sd_ACCpercent_byNum=sd(ACCpercent_byNum),
...
library(ggpubr)
pdf("FigS7.ACC_MRG_proportion.pdf",width = 15,height = 3)
ggarrange(py1_pubAMD + theme(legend.position = "none"),py1_Tailing,py1_ADMSedi,nrow = 1,ncol = 3, widths = c(2.35,4.5,3.05))
dev.off()


pdf("FigS11.ACC_MobilityG_porportion.pdf",width = 15,height = 3)
ggarrange(py2_pubAMD + theme(legend.position = "none"),py2_Tailing,py2_ADMSedi,nrow = 1,ncol = 3, widths = c(2.35,4.5,3.05))
dev.off()
